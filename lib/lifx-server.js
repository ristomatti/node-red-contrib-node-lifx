...
function LightServer(config) {
  ...
  // Ensure that we have valid pollinterval
  if (typeof self.config.interval !== 'number' || isNaN(self.config.interval))
    self.config.interval = 10000;
  // Ensure that we don't use too low poll interval
  else if (self.config.interval < 500)
    self.config.interval = 500;

  // Convert from stirng
  if (typeof self.config.offlineTolerance === 'string')
    self.config.offlineTolerance = parseInt(self.config.offlineTolerance, 10);
  // Ensure that we have valid offline tolerance
  if (typeof self.config.offlineTolerance !== 'number' || isNaN(self.config.offlineTolerance))
    self.config.offlineTolerance = 3;
  // Ensure that we don't use too high offline tolerance
  else if (self.config.offlineTolerance > 50)
    self.config.offlineTolerance = 50;
  ...
  if (_.isNumber(self.config.offlineTolerance))
    this.lifxConfig.lightOfflineTolerance = this.config.offlineTolerance;
